...
class LabelingTransformer(base.BaseEstimator, base.TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for i in range(self.n_features):
            ...
            ft_encoder.fit(X.iloc[:, i].values.tolist() + [self.UNK])
    ...
    def transform(self, X, y = None):
        for i in range(self.n_features):
            ...
            X.iloc[:, i] = X.iloc[:, i].apply(lambda x: str(x) if str(x) in self.encoders[i].classes_.tolist() else self.UNK)
...
class TransformerModelv3(nn.Module):
    ...
    def __init__(self, ninp, nhead, nhid, nlayers, n_num_cols, n_cat_cols, n_cat_labels, n_features, n_classes, dropout=0.5):
        ...
        self.decoder = nn.Linear(ninp * n_features, n_classes)
...
class TransformerModelv4(nn.Module):
    ...
    def __init__(self, ninp, nhead, nhid, nlayers, n_features, n_classes, dropout=0.5):
...
class CustomMetricPipeline(pipeline.Pipeline):

    def __init__(self, score_fn, steps, memory=None, verbose=False):
        self.steps = steps
        self.memory = memory
    ...
    def score(self, X, y, sample_weight=None):
        ...
        return self.score_fn(y, preds)
    


## Helper functions

def load_best_checkpoint(cv, model, dirname=''):
...
def build_inference_pipe(model, preprocessing_pipe, score_fn=metrics.accuracy_score, refit=False):
    
    drop_callbacks = [
        skorch.callbacks.training.Checkpoint, 
        skorch.callbacks.training.EarlyStopping,
        RestoreMoveCheckpoint
    ]
    
    removed_callbacks = []
    
    if refit:
        epochs = model.history[-1]['epoch']    
        model.set_params(train_split=False)
        model.set_params(max_epochs=epochs)
        
        for callback in model.get_params()['callbacks']:
            if type(callback) in drop_callbacks:
                removed_callbacks.append(callback)
                
        for callback in removed_callbacks:
            model.get_params()['callbacks'].remove(callback)
           

    pipe = CustomMetricPipeline(score_fn, [
